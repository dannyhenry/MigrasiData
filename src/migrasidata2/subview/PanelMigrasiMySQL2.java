 ...
 * @author Kaitero
 */
public class PanelMigrasiMySQL2 extends javax.swing.JPanel {
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if (jList2.getSelectedIndex() != -1) {
            ...
            ResultSet rs = db.eksekusiSelect("select " + listKolom + " from " + jList2.getSelectedValue().toString());
            try {
                listData.add(new DataMigrasi(jList2.getSelectedValue().toString(), col, di.loadIsiData2(rs, rs.getMetaData()), di.loadAllKeys2("mysql", jList2.getSelectedValue().toString(), listKolom), listKolom));
    ...
    private void pilihMigrasi(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_pilihMigrasi
        // TODO add your handling code here:
        log = "";
        boolean pilih = false;
        ...
        setDataMigrasi(pilih);
    }//GEN-LAST:event_pilihMigrasi

    public void setDataMigrasi(boolean pilih) {
        if (pilih) {
            fullMigrasi();
        } else {
            customMigrasi();
    ...
    public void readDataMigrasi(ArrayList<DataMigrasi> dm) {
        ...
        for (int i = 0; i < dm.size(); i++) {
            namatabel = cekTabelTujuan(dm.get(i).getNamaTable());
        ...
        for (int i = 0; i < dm.size(); i++) {
            ...
            for (int j = 0; j < listkey.size(); j++) {
                if (listkey.get(j).getJenisKey().equals("PRIMARY KEY")) {
                    ...
                    namaKonstraint = listkey.get(j).getNamaKey();
                }
            }
            if (!kolomkey.equals("")) {
                kolomkey = kolomkey.substring(0, kolomkey.length() - 1);
                query = "alter table " + dm.get(i).getNamaBaru() + " ADD CONSTRAINT " + namaKonstraint + " PRIMARY KEY(" + kolomkey + ")";
                listquery.add(query);
            }
        }
            ...
            try {
                ...
                if (cekposisi) {
                    if (!listquery.get(i).substring(0, 2).equals("##")) {
                        System.out.println(listquery.get(i));
                        boolean hasil = db.eksekusiQueryDMLOracle(listquery.get(i));
                        if (hasil) {
                            sukses++;
                            countSukses++;
                        } else {
